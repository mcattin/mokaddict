...
unsigned long m1CoffeeTimes[timeBufferSize];
unsigned long m2CoffeeTimes[timeBufferSize];
unsigned int m1CoffeeTimesIndex;
unsigned int m2CoffeeTimesIndex;
...
int m1PrevStatCnt, m2PrevStatCnt = 0;
  ...
  switch(m1MainState){
    ...
    case MAKE_COFFEE:
      ...
      if((m1DiffMillis > coffeeWaitTimeout && false == m1CycleStarted) ||
        ...
        if(m1PrevStatCnt != m1StatCnt){
          m1PrevStatCnt = m1StatCnt;
          m1CycleStarted = true;
          
          if(m1CoffeeTimesIndex < timeBufferSize){
            m1CoffeeTimesIndex++;
          }
          else{
            for(int i=0; i < timeBufferSize-1; i++){
              m1CoffeeTimes[i] = m1CoffeeTimes[i+1];
            }
          }
          m1CoffeeTimes[m1CoffeeTimesIndex-1] = m1DiffMillis;
          Serial.print("m1 cnt: ");Serial.print(m1StatCnt, DEC);
          Serial.print(", times[");Serial.print(m1CoffeeTimesIndex-1, DEC);Serial.print("]=");
          Serial.println(m1CoffeeTimes[m1CoffeeTimesIndex-1], DEC);
          
          m1PreviousMillis = currentMillis;
          
          // Check if it's the end of the coffee cycle
          if(m1CoffeeTimesIndex >= 3){
            if((m1CoffeeTimes[m1CoffeeTimesIndex-4] > coffee_1_min) &&
               (m1CoffeeTimes[m1CoffeeTimesIndex-3] > coffee_2_min) &&
               (m1CoffeeTimes[m1CoffeeTimesIndex-3] < coffee_2_max) &&
               (m1CoffeeTimes[m1CoffeeTimesIndex-2] > coffee_3_min) &&
               (m1CoffeeTimes[m1CoffeeTimesIndex-2] < coffee_3_max) &&
               (m1CoffeeTimes[m1CoffeeTimesIndex-1] > coffee_4_min) &&
               (m1CoffeeTimes[m1CoffeeTimesIndex-1] < coffee_4_max)){
  ...
  switch(m2MainState){
    ...
    case MAKE_COFFEE:
      ...
      if((m2DiffMillis > coffeeWaitTimeout && false == m2CycleStarted) ||
        ...
        if(m2PrevStatCnt != m2StatCnt){
          m2PrevStatCnt = m2StatCnt;
          m2CycleStarted = true;
          
          if(m2CoffeeTimesIndex < timeBufferSize){
            m2CoffeeTimesIndex++;
          }
          else{
            for(int i=0; i < timeBufferSize-1; i++){
              m2CoffeeTimes[i] = m2CoffeeTimes[i+1];
            }
          }
          m2CoffeeTimes[m2CoffeeTimesIndex-1] = m2DiffMillis;
          Serial.print("m2 cnt: ");Serial.print(m2StatCnt, DEC);
          Serial.print(", times[");Serial.print(m2CoffeeTimesIndex-1, DEC);Serial.print("]=");
          Serial.println(m2CoffeeTimes[m2CoffeeTimesIndex-1], DEC);
          
          m2PreviousMillis = currentMillis;
          
          // Check if it's the end of the coffee cycle
          if(m2CoffeeTimesIndex >= 3){
            if((m2CoffeeTimes[m2CoffeeTimesIndex-4] > coffee_1_min) &&
               (m2CoffeeTimes[m2CoffeeTimesIndex-3] > coffee_2_min) &&
               (m2CoffeeTimes[m2CoffeeTimesIndex-3] < coffee_2_max) &&
               (m2CoffeeTimes[m2CoffeeTimesIndex-2] > coffee_3_min) &&
               (m2CoffeeTimes[m2CoffeeTimesIndex-2] < coffee_3_max) &&
               (m2CoffeeTimes[m2CoffeeTimesIndex-1] > coffee_4_min) &&
               (m2CoffeeTimes[m2CoffeeTimesIndex-1] < coffee_4_max)){
